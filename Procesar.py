...
def pre_proceso(im_path, path_save, desp_este, desp_norte, desp_yaw):
    ...
    for imp in im_path:
        ...
        # Genera las imágenes para el CVAT, KML y las originales de menor peso
        ...
        #cv2.imwrite(path_save+ "/original_img/"+ name[5:], original_image, [cv2.IMWRITE_JPEG_QUALITY,100])
        cv2.imwrite(path_save + f'/cvat/{imp.split("/")[-1]}', imCVAT)
        cv2.imwrite(path_save + f'/original_img/{imp.split("/")[-1]}', original_image, [cv2.IMWRITE_JPEG_QUALITY, 100])
        cv2.imwrite(path_save + f'/Temp/{imp.split("/")[-1]}', to_uint8g(temperaturas.copy()))
...
def proyectar_imagenes_filtradas(path_save, desp_este, desp_norte, desp_yaw, offset_vuelo, offset_altura, in_vuelo='',
                                mode='Temp',guardar_matriz=False):
    # Si no se recibe el número de vuelo se solicita al usuario a través de la consola
    if in_vuelo == '':
        seleccion = input('''
            ============================================================================
    ...
    path_imagenes = f"{path_save}/original_images/*.JPG"
    ...
    <Folder>
        ...
        for f_name in glob(path_imagenes):
            ...
            # Se actualiza el desplazamiento a los vuelos que correspondan
            if (desp_este != '') and (desp_norte != '') and (desp_yaw != '') and (desp_este != 't') and (desp_norte != 't') and (desp_yaw != 't'):
                if (vuelo == vuelo_sel) or (vuelo_sel == '0'):
                    data2['offset_N'] = desp_norte
                    data2['offset_E'] = desp_este
                    data2['offset_yaw'] = desp_yaw
            ...
            if(guardar_matriz):
                geo_name = path_save + f'/georef_numpy/{f_name.split("/")[-1].replace(".JPG", ".npy")}'
                np.save(geo_name, m)
            p1_ll = utm.to_latlon(m[0][0][0], m[0][0][1], int(m[0][0][2]), string.ascii_uppercase[int(m[0][0][3])])
            p2_ll = utm.to_latlon(m[0][-1][0], m[0][-1][1], int(m[0][-1][2]), string.ascii_uppercase[int(m[0][-1][3])])
            p3_ll = utm.to_latlon(m[-1][-1][0], m[-1][-1][1], int(m[-1][-1][2]),
...
def proyectar_imagenes(path_save, desp_este, desp_norte, desp_yaw, offset_vuelo, offset_altura, in_vuelo='',
                                mode='Temp',guardar_matriz=False):
    # Si no se recibe el número de vuelo se solicita al usuario a través de la consola
    if in_vuelo == '':
        seleccion = input('''
...
def filtro_imagenes_kml(path_save):
    import xml.etree.ElementTree as ET
    import shutil

    # Ruta del archivo KML
    archivo_kml = path_save + "/" + path_save.split('/')[-1] + "_filtro.kml"
    print(archivo_kml)
    ...
    count = 0
    # Iterar sobre cada GroundOverlay para encontrar los enlaces href
    listado_imagenes_kml = list()
    for ground_overlay in root.findall('.//{http://www.opengis.net/kml/2.2}GroundOverlay'):
        count += 1
        # Encontrar y escribir el enlace href en el archivo de texto
        href = ground_overlay.find('.//{http://www.opengis.net/kml/2.2}href').text
        file = href.split('/')[-1]
        listado_imagenes_kml.append(file)

    print("Cantidad de imágenes en KML: ", count)

    # Obtener la lista de archivos en la carpeta 'Temp'
    path_Temp = path_save + "/Temp"
    archivos_en_carpeta = os.listdir(path_Temp)

    # Archivos que serían borrados
    archivos_a_borrar = [archivo.split('.')[0] for archivo in archivos_en_carpeta if archivo not in listado_imagenes_kml]

    path_filtrados = path_save + '/filtrados'
    if not os.path.exists(path_filtrados):
        os.mkdir(path_filtrados)
    if not os.path.exists(path_filtrados + '/Temp'):
        os.mkdir(path_filtrados + '/Temp')
    if not os.path.exists(path_filtrados + '/metadata'):
        os.mkdir(path_filtrados + '/metadata')
    if not os.path.exists(path_filtrados + '/cvat'):
        os.mkdir(path_filtrados + '/cvat')

    # Mueve los archivos que se van a eliminar del procesamiento
    for archivo in archivos_a_borrar:

        ruta_archivo = os.path.join(path_Temp, archivo+'.JPG')
        print(f"Moviendo imagen Temp: {archivo}...")
        shutil.move(ruta_archivo, path_filtrados + '/Temp')

        ruta_archivo = os.path.join(path_save + "/metadata", archivo + '.txt')
        print(f"Moviendo metadata: {archivo}...")
        shutil.move(ruta_archivo, path_filtrados + '/metadata')

        ruta_archivo = os.path.join(path_save + "/cvat", archivo + '.JPG')
        print(f"Moviendo imagen CVAT: {archivo}...")
        shutil.move(ruta_archivo, path_filtrados + '/cvat')
...
if __name__ == '__main__':
    ...
    Elija su opcion:
        ...
        01 Proyectar imágenes (KML)
        02 Filtrar imágenes con KML
        1 Procesar
...
9 Cambiar Directorio:
    ...
    Seleccionar archivo obj.name descargado del cvat            
        ...
        if (seleccion == "100"):
            ...
            proyectar_imagenes(desp_este, desp_norte, desp_yaw, path_save)
        ...
        if (seleccion == "0"):
            ...
            print("Proyectando imágenes y generando KML...")
            proyectar_imagenes(path_save, 0, 0, 0, 0, '0', '0')
            print("Proyecciónes de imágenes listas")
        ...
        if (seleccion == "02"):
            ...
            [im_path, path_save] = inicializar(0)
            #[desp_este, desp_norte, desp_yaw] = desplazamiento()
            print("Filtro de imágenes desde KML..")
            listado_imagenes = filtro_imagenes_kml(path_save)

            # KML sin modificaciones a los desplazamientos y para todos los vuelos
            #generar_kml_imagenes_vuelos(path_save, '', '', '', '', '', '0')
            print("Imágenes filtradas con KML")
        ...
        if (seleccion == "mint"):
            [im_path, path_save] = inicializar(0)
            print("Proyectando imágenes y generando KML...")
            proyectar_imagenes(path_save, 't', 't', 't', '', '', '0', guardar_matriz=True)
            print("Proyecciónes de imágenes listas")
        if (seleccion == "2test"):
            while True:
